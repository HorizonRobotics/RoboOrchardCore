...
- Use `README.md`, `Makefile`, `pyproject.toml`, and
...
- Verify that the repository imports from the local checkout after editable
  ...
  ```bash
  .venv/bin/python -c "import robo_orchard_core; print(robo_orchard_core.__file__)"
  ```
...
- If validation fails because a cache path, shared-memory helper, optional
  dependency, or site package location is not writable, report the
  environment limitation before treating it as a code defect.
