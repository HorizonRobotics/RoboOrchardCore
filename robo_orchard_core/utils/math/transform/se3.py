...
This file is copied from pytorch3d.transforms.se3.py to follow the column-major
convention of rotation matrix.
"""

import torch

from robo_orchard_core.utils.math.transform.so3 import (
    _so3_exp_map,
    hat,
    so3_log_map,
)

__all__ = ["se3_exp_map", "se3_log_map"]


def se3_exp_map(
    log_transform: torch.Tensor, eps: float = 1e-4
) -> torch.Tensor:
...
def _se3_V_matrix(
    ...
    V = (
        ...
        # and `int`.
        * ((1 - torch.cos(rotation_angles)) / (rotation_angles**2))[
            :, None, None
        ]
        + (
            log_rotation_hat_square
...
def _get_se3_V_input(log_rotation: torch.Tensor, eps: float = 1e-4):
    # pyre-fixme[58]: `**` is not supported for operand types `Tensor` and `int`.  # noqa: E501
