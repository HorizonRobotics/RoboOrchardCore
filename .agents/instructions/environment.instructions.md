...
- Otherwise, if this repository root contains `.venv/`, use that
  environment.
- Prefer the user-scoped temporary directory `~/.tmp/` for agent-created
...
- If an import fails while this repository is opened inside a containing
  workspace, confirm the selected interpreter and editable installs before
  treating the missing package as an external dependency problem.
...
- If Hugging Face cache writes are blocked, prefer redirecting the narrowest
  writable cache path needed by the failing tool, such as `HF_DATASETS_CACHE`
  or `HF_HUB_CACHE`, instead of changing `HF_HOME`.
