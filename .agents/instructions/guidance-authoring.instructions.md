...
- Before adding or renaming a local skill or skill family, check for naming
  collisions with globally available skills and existing local skills. Prefer
  specific names for family roots instead of claiming a generic name that may
  already belong to another visible skill surface.
...
- Do not turn a family root into another heavy execution skill when sub-skills
  already own the concrete workflows.
